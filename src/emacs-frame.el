...
`pixel-height', `left', `top', `name', `visibility'."
  (let* ((f (emacs-frame--get frame))
    ...
    (dolist (kv (list (cons 'width (emacs-frame-width f))
                      (cons 'height (emacs-frame-height f))
                      (cons 'pixel-width (emacs-frame-pixel-width f))
                      (cons 'pixel-height (emacs-frame-pixel-height f))
                      (cons 'left (emacs-frame-left f))
                      (cons 'top (emacs-frame-top f))
                      (cons 'name (emacs-frame-name f))
                      (cons 'visibility (emacs-frame-visible f))))
