...
(ert-deftest emacs-load-artifact-cache-test/source-size-gate-skips-compiler-above-limit ()
  (let ((emacs-load-auto-native-compile t)
        (emacs-load-artifact-max-source-size 4)
        (compiler-calls 0)
        (cache-path-calls 0))
    (cl-letf (((symbol-function 'emacs-load--artifact-compiler)
               (lambda ()
                 (setq compiler-calls (1+ compiler-calls))
                 nil))
              ((symbol-function 'emacs-load--artifact-cache-paths)
               (lambda (_resolved)
                 (setq cache-path-calls (1+ cache-path-calls))
                 (error "size gate reached cache path"))))
      (should-not
       (emacs-load--artifact-load-or-compile "oversized.el" "abcde")))
    (should (= compiler-calls 0))
    (should (= cache-path-calls 0))))

(ert-deftest emacs-load-artifact-cache-test/source-size-gate-allows-boundary ()
  (let ((emacs-load-auto-native-compile t)
        (emacs-load-artifact-max-source-size 4)
        (compiler-calls 0))
    (cl-letf (((symbol-function 'emacs-load--artifact-compiler)
               (lambda ()
                 ...
                 nil)))
      (should-not
       (emacs-load--artifact-load-or-compile "eligible.el" "abcd")))
    (should (= compiler-calls 1))))
