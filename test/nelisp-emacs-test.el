;;; nelisp-emacs-test.el --- ERT tests for nelisp-emacs facade  -*- lexical-binding: t; -*-
...
(defconst nelisp-emacs-test--forbidden-library-features
  '(emacs-init
    ...
    emacs-elisp-mode)
  "Features that must stay outside reusable library manifests.")

(defun nelisp-emacs-test--loaded-file-p (basename)
...
(defconst nelisp-emacs-test--expected-stable-text-core-api
  '(nelisp-rx-compile
...
(defconst nelisp-emacs-test--expected-stable-lazy-text-core-api
...
(defconst nelisp-emacs-test--expected-stable-buffer-core-api
...
(defconst nelisp-emacs-test--expected-stable-editing-api
...
(defconst nelisp-emacs-test--expected-stable-io-api
...
(defconst nelisp-emacs-test--expected-stable-special-buffers-api
...
(defconst nelisp-emacs-test--expected-stable-textmodes-stub-api
...
(defconst nelisp-emacs-test--expected-stable-core-api
...
(defun nelisp-emacs-test--feature-manifests ()
  "Return named reusable feature manifests."
  `((nelisp-emacs . ,nelisp-emacs-library-features)
    (emacs-foundation . ,emacs-foundation-features)
    (emacs-text-core . ,emacs-text-core-features)
    (emacs-buffer-core . ,emacs-buffer-core-features)
    (emacs-editing . ,emacs-editing-features)
    (emacs-io . ,emacs-io-features)
    (emacs-core . ,emacs-core-features)))

(defun nelisp-emacs-test--duplicates (items)
  "Return duplicate entries in ITEMS."
  (let (seen duplicates)
    (dolist (item items)
      (if (memq item seen)
...
(ert-deftest nelisp-emacs-test/provides-library-surface ()
  (should (= nelisp-emacs-library-contract-version 1))
  (dolist (feature (cons 'nelisp-emacs nelisp-emacs-library-features))
...
(ert-deftest nelisp-emacs-test/library-package-manifest-is-coherent ()
  ...
  (let ((manifest (nelisp-emacs-library-package-manifest)))
    ...
    (should (equal (mapcar (lambda (entry)
                             (plist-get (cdr entry) :feature))
                           manifest)
                   nelisp-emacs-library-features))
    (dolist (entry manifest)
      (let* ((plist (cdr entry))
             (owner (plist-get plist :owner))
             (feature (plist-get plist :feature))
             (features (plist-get plist :features))
             (lazy-features (plist-get plist :lazy-features)))
        (should (memq owner '(FND TXT BUF CORE IO FEAT)))
        (should (featurep feature))
        (should features)
        (dolist (member-feature features)
          (should (featurep member-feature)))
        (dolist (lazy-feature lazy-features)
          (should (symbolp lazy-feature))
          (should-not
           (nelisp-emacs-test--lazy-feature-loaded-p lazy-feature)))))))

(ert-deftest nelisp-emacs-test/library-package-manifest-returns-copy ()
  (let* ((manifest (nelisp-emacs-library-package-manifest))
         (core (assq 'core manifest))
         (features (plist-get (cdr core) :features))
         (lazy-features (plist-get (cdr core) :lazy-features)))
    (setcar features 'changed-feature)
    (setcar lazy-features 'changed-lazy-feature)
    (should (eq (car emacs-core-features) 'emacs-keymap))
...
(ert-deftest nelisp-emacs-test/library-package-query-api ()
  (should (equal (nelisp-emacs-library-package-names)
                 '(foundation
                   ...
                   core
                   textmodes-stub)))
  (should (equal (plist-get (nelisp-emacs-library-package 'core) :feature)
                 'emacs-core))
  (should (equal (nelisp-emacs-library-package-features 'buffer-core)
                 emacs-buffer-core-features))
  (should (equal (nelisp-emacs-library-package-lazy-features 'text-core)
                 '(nelisp-coding-jis-tables)))
  (should (equal (nelisp-emacs-library-package-lazy-features 'io)
                 '(emacs-dump
                   image-loader
                   nemacs-loaddefs
                   emacs-dired-min
  ...
  (should-not (nelisp-emacs-library-package-lazy-features
               'missing-package))
  (let ((features (nelisp-emacs-library-package-features 'foundation)))
    (setcar features 'changed-feature)
    (should (eq (car emacs-foundation-features) 'emacs-fns)))
...
(ert-deftest nelisp-emacs-test/library-stable-api-query-api ()
  (should (equal (nelisp-emacs-library-stable-api-symbols)
                 (append nelisp-emacs-test--expected-stable-foundation-api
                         nelisp-emacs-test--expected-stable-text-core-api
                         nelisp-emacs-test--expected-stable-buffer-core-api
                         nelisp-emacs-test--expected-stable-editing-api
                         nelisp-emacs-test--expected-stable-io-api
                         nelisp-emacs-test--expected-stable-special-buffers-api
                         nelisp-emacs-test--expected-stable-core-api
                         nelisp-emacs-test--expected-stable-textmodes-stub-api)))
  (should (equal (nelisp-emacs-library-stable-api-symbols 'foundation)
                 nelisp-emacs-test--expected-stable-foundation-api))
  ...
  (should (equal (nelisp-emacs-library-stable-api-symbols 'textmodes-stub)
                 nelisp-emacs-test--expected-stable-textmodes-stub-api))
  (should-not (nelisp-emacs-library-stable-api-symbols 'missing-package))
  (should-not
   (nelisp-emacs-test--duplicates
  ...
  (dolist (symbol (nelisp-emacs-library-stable-api-symbols))
    (let ((entry (nelisp-emacs-library-stable-api-entry symbol)))
      ...
      (should (memq (plist-get entry :package)
                    '(foundation text-core buffer-core editing io
                      special-buffers core textmodes-stub)))
      (should (memq (plist-get entry :owner) '(FND TXT BUF CORE IO FEAT)))
      (should (memq (plist-get entry :package-id)
...
(ert-deftest nelisp-emacs-test/library-stable-lazy-api-query-api ()
  (should (equal (nelisp-emacs-library-stable-lazy-api-symbols)
                 (append
                  nelisp-emacs-test--expected-stable-lazy-text-core-api
                  nelisp-emacs-test--expected-stable-lazy-io-api
                  nelisp-emacs-test--expected-stable-lazy-core-api)))
  ...
  (should (equal (nelisp-emacs-library-stable-lazy-api-symbols 'core)
                 nelisp-emacs-test--expected-stable-lazy-core-api))
  (should-not
   (nelisp-emacs-library-stable-lazy-api-symbols 'missing-package))
  (should-not
   (nelisp-emacs-test--duplicates
    (nelisp-emacs-library-stable-lazy-api-symbols)))
  (dolist (symbol nelisp-emacs-test--expected-stable-lazy-text-core-api)
    (let ((entry (nelisp-emacs-library-stable-lazy-api-entry symbol)))
      (should entry)
      (should (eq (plist-get entry :package) 'text-core))
      (should (eq (plist-get entry :owner) 'TXT))
  ...
  (dolist (symbol nelisp-emacs-test--expected-stable-lazy-core-api)
    (let ((entry (nelisp-emacs-library-stable-lazy-api-entry symbol)))
      ...
      (should (eq (plist-get entry :owner) 'CORE))
      (should (eq (plist-get entry :package-id) 'nelisp-emacs-core))
  ...
  (should-not
   (nelisp-emacs-library-stable-lazy-api-entry 'missing-symbol))
  (let* ((manifest (nelisp-emacs-library-stable-lazy-api-manifest))
         (text-core (assq 'text-core manifest))
         (symbols (plist-get (cdr text-core) :symbols)))
    (setcar (car symbols) 'changed-kind)
    (should (eq (caar (plist-get
                       (cdr (assq
                             'text-core
...
(ert-deftest nelisp-emacs-test/feature-manifests-are-well-formed ()
  (dolist (entry (nelisp-emacs-test--feature-manifests))
    (let ((manifest (cdr entry)))
      (should manifest)
      (should-not (nelisp-emacs-test--duplicates manifest))
      (dolist (feature manifest)
        (should (symbolp feature))
        (should (locate-library (symbol-name feature)))
        (should-not
         (memq feature nelisp-emacs-test--forbidden-library-features))))))

(ert-deftest nelisp-emacs-test/group-loader-manifests-provide-features ()
  (dolist (manifest `(,emacs-foundation-features
...
(ert-deftest nelisp-emacs-test/group-loader-manifests-stay-expected ()
  ...
  (should (equal emacs-editing-features
                 '(emacs-undo
                   emacs-undo-builtins
                   emacs-edit-builtins)))
  (should (equal emacs-core-features
                 '(emacs-keymap
                   emacs-window
                   emacs-minibuffer
...
(ert-deftest nelisp-emacs-test/emacs-init-uses-library-facade ()
  (let ((file (locate-library "emacs-init")))
    (should file)
    (with-temp-buffer
      (insert-file-contents (if (string-match-p "\\.elc\\'" file)
                                (substring file 0 -1)
                              file))
      (should (search-forward "(require 'nelisp-emacs)" nil t))
...
(provide 'nelisp-emacs-test)

;;; nelisp-emacs-test.el ends here
